...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // ── Parse & validate body ────────────────────────────────────────────────
  const { fullName, email, company, phone, country, partnerType, message, recaptchaToken } =
  ...
  try {
    ...
    const internalHtml = buildEmailTemplate(
      "Following partner application received via Ardira Partner Hub",
      [
    ...
    await transporter.sendMail({
      from:     `"Ardira Partnerships" <${TEAM_EMAIL}>`,
      to:        TEAM_EMAIL,
      replyTo:   email,
      subject:  "Partner Application from Ardira website",
      html:      internalHtml,
    });
